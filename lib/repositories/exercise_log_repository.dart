...
class ExerciseLogRepository {
  ...
  void loadExerciseLogs({required List<ExerciseLogDTO> exerciseLogs, required RoutineEditorMode mode}) {
    ...
    for (var exerciseLog in exerciseLogs) {
      if (withDurationOnly(metric: exerciseLog.exerciseVariant.getSetTypeConfiguration())) {
  ...
  List<ExerciseLogDTO> mergeExerciseLogsAndSets() {
    return _exerciseLogs.map((exerciseLog) {
      ...
      return exerciseLog.copyWith(sets: withDurationOnly(metric: exerciseLog.exerciseVariant.getSetTypeConfiguration()) ? _checkSets(sets) : sets);
    }).toList();
  }
